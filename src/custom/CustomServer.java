...
import builtIn.FilesReader;
import builtIn.ThreadPool;
...
public class CustomServer implements Runnable {
    ...
    private CustomFilesReader filesReader;

    private CustomThreadPool poolForIndex;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                future = CompletableFuture.runAsync(() -> {
                    filesReader = new CustomFilesReader();
                    poolForIndex = new CustomThreadPool(cores, filesReader.getIndex());
...
//                    }
    ...
    public void buildAndTestIndexMultipleNumbersOfThreads(List<Integer> cores) {
        for (int i = 0; i < cores.size(); i++) {
            CustomFilesReader filesReader = new CustomFilesReader();
            CustomThreadPool poolForIndex = new CustomThreadPool(cores.get(i), filesReader.getIndex());
            filesReader.readFilesFromDirectory("aclImdb");
            long time = poolForIndex.createInvertedIndexThreadPool(filesReader.getFiles());
